...
// تغيير المسار لإجبار النظام على البدء من الصفر تماماً
const SESSION_PATH = 'session_v92_final'; 
...
async function syncSession() {
    ...
    if (fs.existsSync(credsPath)) {
        ...
        await client.db('whatsapp_bot').collection('session_v92').updateOne({ _id: 'creds' }, { $set: { data } }, { upsert: true });
    }
}
async function restoreSession() {
    if (!dbConnected) return;
    const res = await client.db('whatsapp_bot').collection('session_v92').findOne({ _id: 'creds' });
    if (res) {
...
async function connectToWhatsApp() {
    ...
    sock = makeWASocket({
        ...
        // تغيير الهوية مجدداً لكسر أي تعليق في سيرفرات واتساب
        browser: Browsers.macOS('Mawjat-V92'), 
    ...
    sock.ev.on('connection.update', (u) => {
        ...
        if (connection === 'close') {
            ...
            if (code !== DisconnectReason.loggedOut) setTimeout(connectToWhatsApp, 5000);
                ...
                if(dbConnected) client.db('whatsapp_bot').collection('session_v92').deleteOne({ _id: 'creds' });
            }
        }
    });
...
// التوجيه المباشر للـ Admin وحالة الـ API
...
app.get('/admin', async (req, res) => {
    ...
    res.send(`<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>لوحة التحكم | موجة الصمت</title><script src="https://cdn.tailwindcss.com"></script><style>@import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap');body{font-family:'Cairo',sans-serif;background-color:#f8fafc;}</style></head><body class="p-4 md:p-8 text-right"><div class="max-w-5xl mx-auto space-y-8">
        ...
        <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border text-center space-y-4">
                <h3 class="font-bold text-green-600 text-right">⚙️ الإعدادات</h3>
                <input id="gl" value="${s.googleLink}" class="w-full p-3 bg-gray-100 rounded-xl text-xs border text-center">
                <div class="flex gap-2">
                    <input id="dc" value="${s.discountCode}" class="w-1/2 p-3 bg-gray-50 rounded-xl text-center font-bold text-blue-600 border">
                    <input id="dl" value="${s.delay}" class="w-1/2 p-3 bg-gray-100 rounded-xl text-center font-bold border">
                </div>
                <button onclick="save()" class="w-full bg-black text-white p-4 rounded-2xl font-bold">حفظ</button>
            </div>
        </div>

        <div id="qr-container" class="bg-white p-8 rounded-[2.5rem] border-2 border-dashed border-gray-100 flex flex-col items-center justify-center min-h-[300px]">
            <p class="text-gray-400 animate-pulse font-bold">بانتظار الباركود...</p>
        </div>
